...
class UploadService extends Service {
  ...
  async index(payload) {
    ...
    if (isPaging) {
      if (search) {
        if (kind) {
          res = await this.ctx.model.Attachment.find({ filename: { $regex: search }, extname: { $in: attachmentKind[`${kind}`] } })
        ...
        if (kind) {
          res = await this.ctx.model.Attachment.find({ extname: { $in: attachmentKind[`${kind}`] } })
            ...
            .sort({ createdAt: -1 })
            .exec();
          count = await this.ctx.model.Attachment
            .countDocuments({ extname: { $in: attachmentKind[`${kind}`] } })
            .exec();
        } else {
